...
/**
 * /review/[id] — full review of a past game.
 *
 * Fetches the stored game (GET /api/games/[id]), replays session_moves into
 * the shared review pipeline (useGameReview → Stockfish evals + move
 * classification + Claude commentary) and renders the ONE shared review UI
 * (components/shared/GameReview) with classification badges. Linked from the
 * Recent Games list on /profile.
 */
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const reviewMoves: ReviewMove[] = (data.moves ?? [])
          .filter((m: Record<string, unknown>) => m.san && m.from && m.to && m.fenAfter)
