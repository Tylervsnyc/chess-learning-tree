...
export default function PreviewLessonPage() {
  ...
  // Progress stats - use first attempt results for final score
  ...
  const recordAndAdvance = useCallback((result: 'correct' | 'wrong') => {
    ...
    if (!inRetryMode) {
      ...
      // No Supabase tracking in preview mode - localStorage only
    }
  ...
  }, [currentPuzzle, currentIndex, totalPuzzles, inRetryMode, retryQueue, puzzles, results, firstAttemptCorrectCount]);
  ...
  // Save completion to localStorage only (no Supabase in preview)
  useEffect(() => {
    if (lessonComplete) {
      const stored = localStorage.getItem(PREVIEW_STORAGE_KEY);
      const completed = stored ? new Set(JSON.parse(stored)) : new Set();
      completed.add(lessonId);
      localStorage.setItem(PREVIEW_STORAGE_KEY, JSON.stringify([...completed]));
    }
      ...
      <div className="h-screen bg-[#131F24] text-white flex flex-col overflow-hidden">
        ...
        <div className="bg-[#1A2C35] border-b border-white/10 px-4 py-3 flex-shrink-0">
          <div className="max-w-4xl mx-auto flex items-center justify-between">
            <button
              onClick={() => router.push(`/preview/learn?level=${level}`)}
