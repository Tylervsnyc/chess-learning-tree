          ...
          <div className="space-y-2">
            {isLoggedIn ? (
              <>
                ...
                <Link
                  href={`/staging/level-test/${testTransition}`}
              ...
              <>
                ...
                <Link
                  href="/auth/login?redirect=/staging/learn"
                  className="inline-block px-6 py-3 rounded-xl font-bold text-white transition-all hover:scale-105"
                  style={{
                    backgroundColor: levelColor,
                    boxShadow: `0 4px 0 ${darkColor}`,
...
export default function LearnPage() {
  ...
  // Track global section index across all levels
  let globalSectionIndex = 0;

  return (
    <div className="min-h-screen text-[#3c3c3c] pb-20">
      <style>{`
        @keyframes sparkle {
      ...
      <div className="max-w-lg mx-auto px-4 py-6">
        {LEVELS.map(({ level, data, color, darkColor }, levelIndex) => {
          ...
          // Calculate starting section index for this level
          const levelStartSectionIndex = globalSectionIndex;
...
function LessonButton({
  ...
  return (
    <Link href={`/staging/lesson/${lesson.id}`}>
      {buttonContent}
    </Link>
  );
}
