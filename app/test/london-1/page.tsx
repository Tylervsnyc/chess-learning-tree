...
function isUnlocked(nodeId: string, completed: Set<string>): boolean {
  const node = LONDON_SYSTEM.nodes.find(n => n.id === nodeId)
...
export default function London1TestPage() {
  const router = useRouter()
  const [completedIds, setCompletedIds] = useState<string[]>([])
  const [currentId, setCurrentId] = useState<string | null>('ln-1')
  ...
  const completeNext = useCallback(() => {
    ...
    const idx = LONDON_SYSTEM.completionOrder.indexOf(currentId)
    let nextId: string | null = null
    for (let i = idx + 1; i < LONDON_SYSTEM.completionOrder.length; i++) {
      const candidateId = LONDON_SYSTEM.completionOrder[i]
    ...
    if (!nextId) {
      const fallback = LONDON_SYSTEM.nodes.find(n => !completedSet.has(n.id) && isUnlocked(n.id, completedSet))
  ...
  const completeAll = useCallback(() => {
    setCompletedIds(LONDON_SYSTEM.nodes.map(n => n.id))
    setCurrentId(null)
    setTreeKey(k => k + 1)
  }, [])
    ...
    <div className="min-h-screen bg-chess-page flex items-center justify-center overflow-auto p-4">
      <div className="flex flex-col overflow-hidden" style={{
        ...
        <div className="flex-shrink-0 border-b border-gray-200 px-4 py-3 flex items-center gap-3" style={{ background: 'var(--color-chess-page)' }}>
          ...
          <div>
            <h1 className="text-lg font-bold" style={{ color: 'var(--color-chess-text)' }}>London System</h1>
            <p className="text-xs" style={{ color: 'var(--color-chess-text-muted)' }}>
              {completedIds.length} / {LONDON_SYSTEM.nodes.length} lessons
            </p>
          </div>
        </div>
        ...
        <OpeningTreeView
          ...
          onLessonTap={(nodeId) => {
            if (nodeId === currentId) {
              ...
              router.push(`/openings/london/${nodeId}`)
            }
          }}
        />
