...
function isUnlocked(nodeId: string, completed: Set<string>): boolean {
  const node = CARO_KANN.nodes.find(n => n.id === nodeId)
...
export default function CaroKann1TestPage() {
  const router = useRouter()
  const [completedIds, setCompletedIds] = useState<string[]>([])
  const [currentId, setCurrentId] = useState<string | null>('ck-1')
  ...
  const completeNext = useCallback(() => {
    ...
    const idx = CARO_KANN.completionOrder.indexOf(currentId)
    let nextId: string | null = null
    for (let i = idx + 1; i < CARO_KANN.completionOrder.length; i++) {
      const candidateId = CARO_KANN.completionOrder[i]
    ...
    if (!nextId) {
      const fallback = CARO_KANN.nodes.find(n => !completedSet.has(n.id) && isUnlocked(n.id, completedSet))
  ...
  const completeAll = useCallback(() => {
    setCompletedIds(CARO_KANN.nodes.map(n => n.id))
    setCurrentId(null)
    setTreeKey(k => k + 1)
  }, [])
    ...
    <div className="min-h-screen bg-chess-page flex items-center justify-center overflow-auto p-4">
      <div className="flex flex-col overflow-hidden" style={{
        ...
        <div className="flex-shrink-0 border-b border-gray-200 px-4 py-3 flex items-center gap-3" style={{ background: 'var(--color-chess-page)' }}>
          ...
          <div>
            <h1 className="text-lg font-bold" style={{ color: 'var(--color-chess-text)' }}>Caro-Kann Defense</h1>
            <p className="text-xs" style={{ color: 'var(--color-chess-text-muted)' }}>
              {completedIds.length} / {CARO_KANN.nodes.length} lessons
            </p>
          </div>
        </div>
        ...
        <OpeningTreeView
          ...
          onLessonTap={(nodeId) => {
            if (nodeId === currentId) {
              ...
              router.push(`/openings/caro-kann/${nodeId}`)
            }
          }}
        />
