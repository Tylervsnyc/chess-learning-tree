...
function isUnlocked(nodeId: string, completed: Set<string>): boolean {
  const node = PIRC_AUSTRIAN.nodes.find(n => n.id === nodeId)
...
export default function PircAustrianTestPage() {
  const router = useRouter()
  const [completedIds, setCompletedIds] = useState<string[]>([])
  const [currentId, setCurrentId] = useState<string | null>('pa-1')
  ...
  const completeNext = useCallback(() => {
    ...
    const idx = PIRC_AUSTRIAN.completionOrder.indexOf(currentId)
    let nextId: string | null = null
    for (let i = idx + 1; i < PIRC_AUSTRIAN.completionOrder.length; i++) {
      const candidateId = PIRC_AUSTRIAN.completionOrder[i]
    ...
    if (!nextId) {
      const fallback = PIRC_AUSTRIAN.nodes.find(n => !completedSet.has(n.id) && isUnlocked(n.id, completedSet))
  ...
  const completeAll = useCallback(() => {
    setCompletedIds(PIRC_AUSTRIAN.nodes.map(n => n.id))
    setCurrentId(null)
    setTreeKey(k => k + 1)
  }, [])
    ...
    <div className="min-h-screen bg-chess-page flex items-center justify-center overflow-auto p-4">
      <div className="flex flex-col overflow-hidden" style={{
        ...
        <div className="flex-shrink-0 border-b border-gray-200 px-4 py-3 flex items-center gap-3" style={{ background: 'var(--color-chess-page)' }}>
          ...
          <div>
            <h1 className="text-lg font-bold" style={{ color: 'var(--color-chess-text)' }}>Pirc Austrian Attack</h1>
            <p className="text-xs" style={{ color: 'var(--color-chess-text-muted)' }}>
              {completedIds.length} / {PIRC_AUSTRIAN.nodes.length} lessons
            </p>
          </div>
        </div>
        ...
        <OpeningTreeView
          ...
          onLessonTap={(nodeId) => {
            router.push(`/openings/pirc-austrian/${nodeId}`)
          }}
        />
