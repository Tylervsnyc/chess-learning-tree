...
function isUnlocked(nodeId: string, completed: Set<string>): boolean {
  const node = FRENCH_DEFENSE.nodes.find(n => n.id === nodeId)
...
export default function French1TestPage() {
  const router = useRouter()
  const [completedIds, setCompletedIds] = useState<string[]>([])
  const [currentId, setCurrentId] = useState<string | null>('fr-1')
  ...
  const completeNext = useCallback(() => {
    ...
    const idx = FRENCH_DEFENSE.completionOrder.indexOf(currentId)
    let nextId: string | null = null
    for (let i = idx + 1; i < FRENCH_DEFENSE.completionOrder.length; i++) {
      const candidateId = FRENCH_DEFENSE.completionOrder[i]
    ...
    if (!nextId) {
      const fallback = FRENCH_DEFENSE.nodes.find(n => !completedSet.has(n.id) && isUnlocked(n.id, completedSet))
  ...
  const completeAll = useCallback(() => {
    setCompletedIds(FRENCH_DEFENSE.nodes.map(n => n.id))
    setCurrentId(null)
    setTreeKey(k => k + 1)
  }, [])
    ...
    <div className="min-h-screen bg-chess-page flex items-center justify-center overflow-auto p-4">
      <div className="flex flex-col overflow-hidden" style={{
        ...
        <div className="flex-shrink-0 border-b border-gray-200 px-4 py-3 flex items-center gap-3" style={{ background: 'var(--color-chess-page)' }}>
          ...
          <div>
            <h1 className="text-lg font-bold" style={{ color: 'var(--color-chess-text)' }}>French Defense</h1>
            <p className="text-xs" style={{ color: 'var(--color-chess-text-muted)' }}>
              {completedIds.length} / {FRENCH_DEFENSE.nodes.length} lessons
            </p>
          </div>
        </div>
        ...
        <OpeningTreeView
          ...
          onLessonTap={(nodeId) => {
            if (nodeId === currentId) {
              ...
              router.push(`/openings/french/${nodeId}`)
            }
          }}
        />
