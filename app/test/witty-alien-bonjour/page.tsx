...
function isUnlocked(nodeId: string, completed: Set<string>): boolean {
  const node = WITTY_ALIEN_BONJOUR.nodes.find(n => n.id === nodeId)
...
export default function WittyAlienBonjourTestPage() {
  const router = useRouter()
  const [completedIds, setCompletedIds] = useState<string[]>([])
  const [currentId, setCurrentId] = useState<string | null>('wab-1')
  ...
  const completeNext = useCallback(() => {
    ...
    const idx = WITTY_ALIEN_BONJOUR.completionOrder.indexOf(currentId)
    let nextId: string | null = null
    for (let i = idx + 1; i < WITTY_ALIEN_BONJOUR.completionOrder.length; i++) {
      const candidateId = WITTY_ALIEN_BONJOUR.completionOrder[i]
    ...
    if (!nextId) {
      const fallback = WITTY_ALIEN_BONJOUR.nodes.find(n => !completedSet.has(n.id) && isUnlocked(n.id, completedSet))
  ...
  const completeAll = useCallback(() => {
    setCompletedIds(WITTY_ALIEN_BONJOUR.nodes.map(n => n.id))
    setCurrentId(null)
    setTreeKey(k => k + 1)
  }, [])
    ...
    <div className="min-h-screen bg-chess-page flex items-center justify-center overflow-auto p-4">
      <div className="flex flex-col overflow-hidden" style={{
        ...
        <div className="flex-shrink-0 border-b border-gray-200 px-4 py-3 flex items-center gap-3" style={{ background: 'var(--color-chess-page)' }}>
          ...
          <div>
            <h1 className="text-lg font-bold" style={{ color: 'var(--color-chess-text)' }}>Bonjour Variation</h1>
            <p className="text-xs" style={{ color: 'var(--color-chess-text-muted)' }}>
              {completedIds.length} / {WITTY_ALIEN_BONJOUR.nodes.length} lessons
            </p>
          </div>
        </div>
        ...
        <OpeningTreeView
          key={treeKey}
          tree={WITTY_ALIEN_BONJOUR}
          completedLessonIds={completedIds}
          currentLessonId={currentId}
          onLessonTap={(nodeId) => {
            router.push(`/openings/witty-alien-bonjour/${nodeId}`)
          }}
        />
