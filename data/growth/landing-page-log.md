...
| 3 | `IG_LANDING_COPY` | ~2026-06-05 (D3) | Copy-only swap inside `ColdLanding`: challenge-framed headline echoing the ad hook ("Beat me in 60 seconds?", "Play me now") instead of the "learn" promise — reframes work → game | _pending_ | _pending_ |
| 5 | `IG_LANDING_CHECKMATE` | _building 2026-06-05_ (D5) | Drop cold IG **straight onto the ad's mate-in-1 board** (`/lesson/1.1.1`'s PUZZLE_1: Qd3→h7#, h7 glows green). Tap queen → tap glowing square → win → signup → continue lesson 1.1.1. New component `CheckmateLanding.tsx`; highest precedence. | _pending_ | _pending_ |
