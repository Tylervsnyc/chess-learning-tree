/**
 * Send the Knicks Finals takeover broadcast to all users who haven't already
 * received it. Respects marketing preference + unsubscribe.
 *
 * Usage:
 *   npx tsx scripts/send-knicks-email.ts                  # dry-run (default)
 *   npx tsx scripts/send-knicks-email.ts --preview        # write + open HTML preview
 *   npx tsx scripts/send-knicks-email.ts --test=you@x.com # send ONE test
 *   npx tsx scripts/send-knicks-email.ts --send           # live to everyone
 */
...
import { KnicksTakeover } from '../lib/email/templates/KnicksTakeover';

const TEST_ARG = process.argv.find((a) => a.startsWith('--test='));
const TEST_EMAIL = TEST_ARG ? TEST_ARG.split('=')[1] : null;
const PREVIEW = process.argv.includes('--preview');
const DRY_RUN = !process.argv.includes('--send') && !TEST_EMAIL && !PREVIEW;
const BATCH_DELAY_MS = 500;
const EMAIL_TYPE = 'knicks_takeover' as const;
const SUBJECT = 'Rookie is a Knicks fan now';
...
async function main() {
  ...
  if (PREVIEW) {
    ...
    writeFileSync('/tmp/knicks-email-preview.html', html);
    console.log('Preview written to /tmp/knicks-email-preview.html');
    return;
  }
  ...
  if (TEST_EMAIL) {
    ...
    const result = await sendEmail({
      ...
      type: EMAIL_TYPE,
      subject: SUBJECT,
      react: KnicksTakeover({ displayName, appUrl, unsubscribeUrl }),
      metadata: { test: true },
    });
