/**
 * Send the Rookie's Run launch email to all users who haven't already received it.
 * Respects marketing preference.
 *
 * Usage:
 *   npx tsx scripts/send-rookies-run-launch.ts           # dry-run (default)
 *   npx tsx scripts/send-rookies-run-launch.ts --send    # actually send
 */
...
const EMAIL_TYPE = 'rookies_run_launch';
const SUBJECT = "Rookie's Run -- she's got this. (she does not.)";
...
async function main() {
  ...
  for (const user of eligible) {
    ...
    const result = await sendEmail({
      ...
      type: EMAIL_TYPE,
      subject: SUBJECT,
      react: RookiesRunLaunch({ displayName, appUrl, unsubscribeUrl }),
    });
