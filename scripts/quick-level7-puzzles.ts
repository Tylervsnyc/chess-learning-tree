/**
 * Quick puzzle generator for Level 7
 */
  ...
  for (const file of files) {
    try {
      ...
      for (const line of lines) {
        ...
        if (rating < 1800 || rating > 2000) continue;
...
async function main() {
  console.log('Quick Level 7 puzzle generator\n');
  ...
  const filtered = existing.filter((e: any) => !e.lessonId.startsWith('7.'));
  console.log(`Keeping ${filtered.length} existing entries\n`);

  const allLessons = level7.modules.flatMap(m => m.lessons);
