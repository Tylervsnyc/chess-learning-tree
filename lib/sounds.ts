...
// ---------------------------------------------------------------------------
// Achievement pop-up SFX (Chess Boxing medals). Files generated by
// scripts/gen-achievement-sfx.ts; audition + pick at /test/achievement-sfx.
// Loaded lazily on first play — nothing is fetched on page load.
// ---------------------------------------------------------------------------

export type AchievementSfxKind = 'toast' | 'plaque' | 'ceremony' | 'upgrade' | 'shame';

/** Chosen variant per moment. Swap a filename here to apply a pick from /test/achievement-sfx. */
export const ACHIEVEMENT_SFX_FILES: Record<AchievementSfxKind, string> = {
  toast: 'toast-1.mp3', // S — "the nod": medal clink + brass sting
  plaque: 'plaque-1.mp3', // M — "new belt": ring bell + crowd swell
  ceremony: 'ceremony-1.mp3', // L — "title fight": bell + fanfare + roar
  upgrade: 'upgrade-1.mp3', // tier upgraded: belt-notch ratchet + chime
  shame: 'shame-1.mp3', // secret shame / roast medal: sad trombone
};

const SOUND_ENABLED_KEY = 'cp_sound_enabled';

/** Global app sound toggle (default on). Persisted in localStorage. */
...
const achievementSfxCache = new Map<string, AudioBuffer>();

/**
 * Play an achievement pop-up sound. `file` overrides the chosen variant
 * (used by the audition page). Silent when sound is disabled.
 */
export async function playAchievementSfx(kind: AchievementSfxKind, file?: string): Promise<void> {
  if (!isSoundEnabled()) return;
  const ctx = await ensureAudioReady();
  if (!ctx) return;
  const url = `/sounds/achievements/${file ?? ACHIEVEMENT_SFX_FILES[kind]}`;
  let buffer = achievementSfxCache.get(url);
  if (!buffer) {
    try {
      const res = await fetch(url);
      if (!res.ok) {
        console.warn('[playAchievementSfx] missing file', url);
        return;
      }
      buffer = await ctx.decodeAudioData(await res.arrayBuffer());
      achievementSfxCache.set(url, buffer);
    } catch (err) {
      console.warn('[playAchievementSfx] failed', err);
      return;
    }
  }
  const source = ctx.createBufferSource();
