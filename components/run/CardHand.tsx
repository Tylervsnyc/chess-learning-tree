...
export function CardHand({ hand, onPlay }: CardHandProps) {
...
function CardSlot({ cardId, onClick }: CardSlotProps) {
  if (!cardId) {
    return (
  ...
  return (
    <button
      ...
      onClick={onClick}
      title={def.blurb}
      className="flex-1 h-9 rounded-lg bg-gradient-to-br from-indigo-100 to-indigo-50 dark:from-indigo-900/40 dark:to-indigo-950/40 border border-indigo-300 dark:border-indigo-700 shadow-sm px-2 flex items-center justify-center gap-1.5 text-indigo-700 dark:text-indigo-300 active:scale-[0.97] transition-transform"
    >
