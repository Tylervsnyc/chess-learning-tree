 ...
 * /play (logged in — middleware sends / there; /path is the legacy target).
 * This sends native users to /box ONCE per session, so a later in-app visit
 * to those routes (e.g. the Play tab, browsing lessons from Train) is left
 * alone. Web visitors are untouched; debug with ?boxapp=1.
 */

const REDIRECTED_KEY = 'cp:box-home-redirected';
const COLD_START_ROUTES = ['/', '/welcome', '/path', '/play'];
