...
/**
 * WorkoutReport — the interactive post-workout report.
 *
 * Rendered by two thin routes: /workout/report?id=<sessionId> (the one form
 * every link in the app uses — it's the only shape the offline iOS bundle can
 * static-export, same as /review?id=) and the legacy /workout/report/[id]
 * (kept alive for emails already sent).
 *
 ...
 *
 * Pro (Gate B, FEATURE_FLAGS.PRO): miss #1 is the free preview. Misses 2+
 * keep the board + evals but Rookie's line is a locked row, and the Focused
 * Workout (Fix-It) button goes through `requirePro('fixit')`. The server
 * decides what was withheld (`report.locked`); this only renders it.
 */
...
/** The canonical link to a report — one URL form everywhere. */
export function workoutReportHref(sessionId: string): string {
  return `/workout/report?id=${encodeURIComponent(sessionId)}`;
}

export function WorkoutReport({ sessionId }: { sessionId: string }) {
  ...
  const report = useMissAnalysis(missed, id, { isPro: FEATURE_FLAGS.PRO && isPro });
  const { analyses, lines, diagnosis, profile } = report;
  const lineLoading = report.status === 'engine' || report.status === 'rookie';
  // Rookie's lines past miss #1 were withheld by the server (never while PRO is off).
  const linesLocked = FEATURE_FLAGS.PRO && report.locked;
  const unlockReport = useCallback(() => requirePro('report', () => {}), [requirePro]);
      ...
      <Shell>
        ...
        <div className="flex-1 min-h-0 relative z-10 overflow-y-auto ring-scroll pt-[max(4rem,calc(env(safe-area-inset-top)+3.5rem))] pb-[max(1rem,env(safe-area-inset-bottom))]">
          <div className="max-w-md mx-auto w-full px-5 py-6 flex flex-col items-center text-center gap-5">
            ...
            {signin ? (
              <Link href={`/auth/login?redirect=${encodeURIComponent(workoutReportHref(id ?? ''))}`} className="w-full max-w-xs">
  ...
  if (screen.kind === 'miss') {
    ...
    body = (
      <>
        ...
        <MissReplay
          key={`${a.puzzleId}-${screen.index}`}
          analysis={a}
          line={lines[screen.index]}
          lineLoading={lineLoading}
          locked={linesLocked && screen.index > 0}
      ...
      <>
        ...
        <div className="shrink-0 flex flex-col gap-2">
          ...
          <div className="flex items-center justify-between">
            ...
            {/* /workout/review/[id] is a dynamic route the offline bundle
                can't export — only offer it where it exists. */}
            {!IS_OFFLINE_APP && (
              <Link
