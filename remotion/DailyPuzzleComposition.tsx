...
/**
 * The daily puzzle reel composition — ONE definition, registered by both the
 * studio Root (Root.tsx) and the renderer's slim entry (daily-puzzle-entry.ts).
 */
export const DailyPuzzleComposition: React.FC = () => (
  <Composition
    id="DailyPuzzleVideo"
    ...
    durationInFrames={totalFrames(defaultProps.rawMoves.length - 1)}
    fps={FPS}
    width={FRAME_W}
    height={FRAME_H}
    ...
    calculateMetadata={({ props }) => {
      ...
      return { durationInFrames: totalFrames(p.rawMoves.length - 1) };
    }}
  />
);
