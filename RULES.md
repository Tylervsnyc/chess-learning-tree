...
- **Experimental BOARD-EDITING abilities (bot-only, not offered to players):** `smoke` and `boulder` join `detonate`/`yank`/`phalanx` in `EXPERIMENTAL_ABILITY_IDS`. They add terrain to `BoardState` instead of acting on pieces: `smoke` (Instant · Cloud) puffs a cloud centered on Rookie (`state.smoke`, T1 1×1/1 turn → T5 5×5/3 turns; enemies can't see through, enter, or capture on it; Rookie/allies move freely; ticks down at the end of each full enemy turn); `boulder` (Targeted · Rock) drops a rock within Chebyshev range 1–3 (`state.boulders`, persists for the level; blocks EVERYONE — Rookie rays stop before it, knight-form may jump over; T5 may crush an enemy). Both are cleared at level start. Existing `hazards` are NOT reused. Playtest via `scripts/run-playtest/ability-power-test.ts`.
