...
- **One authority:** `lib/rookie/win-ladder.ts`. Derived on every read, like the streak — no stored counter, no columns. The server replays the user's win history chronologically (`game_sessions` rows with `result='win'` + `rookie_difficulty`, plus `bout_sessions` wins), counting each win toward the then-current derived level. **A win only counts if it was played AT (or above) the then-current level** — users can tap the /play level bar to replay any unlocked lower level, but those "victory lap" wins never advance the ladder (rule enforced in the replay, the POST fold, and the anon localStorage path). This applies retroactively to historical games.
...
- **BOARD-EDITING abilities:** `smoke` is SHIPPED (in the offer pool since 2026-08-18 — power test T4×12: +19.2/+30.0/+37.5pp at T1/T3/T5, above freeze-ray at every tier; card art `public/abilities/smoke-1.webp`, board renders the cloud as a grey wash in `Board.tsx`). `boulder` stays EXPERIMENTAL (bot-only, in `EXPERIMENTAL_ABILITY_IDS` with `detonate`/`yank`/`phalanx`) — after the T1/T3 buff it still measures +8.3/+11.7pp at T1/T3 (freeze-ray = 15.0/22.5), below the weakest shipped ability by more than the 5pp band; T5 +37.5pp is fine, so the low tiers need a mechanic rework, not just numbers. Both add terrain to `BoardState` instead of acting on pieces: `smoke` (Instant · Cloud) puffs a cloud centered on Rookie (`state.smoke`, T1 3×3/1 turn → T3 3×3/3 turns → T5 5×5/4 turns; uses 1/1/2/2/3; enemies can't see through, enter, or capture on it; Rookie/allies move freely; ticks down at the end of each full enemy turn); `boulder` (Targeted · Rock) drops a rock within Chebyshev range 2 (T1–2) or 3 (T3–5) (`state.boulders`, persists for the level; blocks EVERYONE — Rookie rays stop before it, knight-form may jump over; rocks/level 2/2/3/3/4; T4 may crush a PAWN, T5 may crush any enemy). Both are cleared at level start. Existing `hazards` are NOT reused. Playtest via `scripts/run-playtest/ability-power-test.ts`.
