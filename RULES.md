...
**Stage 4 (Celebrate):** Final position with result overlay on board (dark semi-transparent badge, 72px white text). Result is auto-generated: "Checkmate in N!", "Won the Queen!", "Won a Piece!", "Brilliant Move!", etc. **Material headlines use NET material for the solver over the whole solution** (`describeResult` in `remotion/lib/describe-result.ts`, trades cancelled, promotions counted): a headline never claims more than the net gain and **a trade is never a win** ("Queen for a Rook!", "Won the Exchange!"); non-mate lines that end level or down get positional wording ("Winning Position!", "Decisive Attack!", "Unstoppable Pawn!"). `scripts/ig-verify-captions.ts` recomputes every headline and fails on any overclaim. The bottom card leads with the **payoff line** — the position-derived explanation of why the solution works (`lib/ig-puzzle-insight.ts`, the same line the caption uses below its spoiler gap) — with the quip demoted underneath it. Stage 4 runs 5s because 3s was not enough time to read the explanation.
